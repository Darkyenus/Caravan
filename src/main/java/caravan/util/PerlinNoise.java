...
public final class PerlinNoise {
    ...
    public float sample(float x, float y, int octavePower, float magnitude){
        ...
        seed(firstX,firstY);
        float c00 = r.nextFloat() * 2f - 1f;

        seed(secondX, firstY);
        float c10 = r.nextFloat() * 2f - 1f;

        seed(secondX, secondY);
        float c11 = r.nextFloat() * 2f - 1f;

        seed(firstX, secondY);
        float c01 = r.nextFloat() * 2f - 1f;

        return i(i(c00, c10, xA), i(c01, c11, xA), yA) * magnitude;
    }

    public float sample(float x, float y){
        return (1f + sample(x,y,256,1f/2f) + sample(x,y,128,1f/4f) + sample(x,y,64,1f/8f) + sample(x,y,32,1f/16f))/2f;
    }

    public float sampleDense(float x, float y){
        return (1f + sample(x,y,64,1f/2f) + sample(x,y,32,1f/4f) + sample(x,y,16,1f/8f) + sample(x,y,8,1f/16f))/2f;
    }

    public float sampleVeryDense(float x, float y){
        return (1f + sample(x,y,32,1f/2f) + sample(x,y,16,1f/4f) + sample(x,y,8,1f/8f) + sample(x,y,4,1f/16f))/2f;
    }
